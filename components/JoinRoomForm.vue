...
<template>
  <form id="join-room-form" class="flex gap-4" @submit.prevent="submit">
    <input v-model="roomCode" type="text" required name="roomCode" placeholder="Enter room code or link"
      autocomplete="off" spellcheck="false" :disabled="checking"
      class="w-full bg-dark-100 border border-gray-600 rounded-xl px-4 py-4 text-white placeholder-gray-400 focus:outline-none focus:border-blue-500 focus:ring-1 focus:ring-blue-500 disabled:opacity-60">
    <button :disabled="checking"
      class="flex items-center w-auto justify-center gap-3 bg-gray-700 whitespace-nowrap hover:bg-gray-600 text-white font-normal hover:cursor-pointer py-4 px-6 rounded-xl transition-colors disabled:opacity-60 disabled:cursor-wait" type="submit">
      <UIcon v-if="checking" name="i-lucide-loader-circle" class="text-base animate-spin" />
      <svg v-else width="16" height="16" class="svg-inline--fa fa-right-to-bracket" aria-hidden="true" data-prefix="fas"
        data-icon="right-to-bracket" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 512 512">
      ...
      <span>{{ checking ? 'Checking…' : 'Join Room' }}</span>
    </button>
  </form>
</template>
